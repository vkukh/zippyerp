...
class Account extends \ZCL\DB\Entity
{
        ...
        public function getSaldo()
        {
                ...
                $conn = DB::getConnect();
                $sql = "select coalesce(sum(amount),0) from  erp_account_entry_view where  acc_d=" . $this->acc_id;
                ;
                $deb = $conn->GetOne($sql);
                $sql = "select coalesce(sum(amount),0) from  erp_account_entry_view where  acc_c=" . $this->acc_id;
                ;
                $cr = $conn->GetOne($sql);
