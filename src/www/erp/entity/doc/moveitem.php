...
class MoveItem extends Document
{
        ...
        public function ExecuteImpl()
        {
                foreach ($this->detaildata as $value) {
                        ...
                        $stock->updateStock(0 - $value['quantity'],0, $this->document_id, strlen($value['serial_number']) > 0 ? array($value['serial_number']) : array());

                        $stock = Stock::getStock($this->headerdata['storeto'], $value['item_id'], $value['price'], true);
                        $stock->updateStock($value['quantity'],0, $this->document_id, strlen($value['serial_number']) > 0 ? array($value['serial_number']) : array());
                }
                return true;
        }
