...
class TaxInvoice extends Document
{
    ...
    public function generateReport()
    {
        ...
        $contract = Document::load($this->headerdata["based"])  ;
        ...
        $header = array('date' => date('dmY', $this->document_date),
            "firmname" => $firm['name'],
            "firmcode" => H::addSpaces($firm['inn']),
            "customername" => $customer->customer_name,
            "customercode" => H::addSpaces($customer->inn),
            "saddress" => $firm['street'] .',' . $firm['city'],
            "baddress" => $customer->street .',' . $customer->city,
            "sphone" => H::addSpaces($firm['phone']),
            "bphone" => H::addSpaces($customer->phone),
  ...
  $xml .= "<SOFTWARE>Zippy ERP</SOFTWARE>";      
     ...
     if(strlen($firm['phone']) > 0)$xml .="<HTELSEL>{$firm['phone']}</HTELSEL>"; 
     ...
     if(strlen($this->headerdata['based']) > 0)$xml .="<H01G1S>{$this->headerdata['based']}</H01G1S>"; 
     if(strlen($this->headerdata['ernn']) == true)$xml .="<HERPN>true</HERPN>"; 
     ...
     foreach ($this->detaildata as $value){
         ...
         if(strlen($value['uktzed'] > 0))$xml .="<RXXXXG4  ROWNUM=\"{$num}\">{$value['uktzed']}</RXXXXG4>";
