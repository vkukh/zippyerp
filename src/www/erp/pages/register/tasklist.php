...
class TaskList extends \ZippyERP\ERP\Pages\Base
{
    ...
    public function __construct($task_id = 0) {
        ...
        $this->contenttab->add(new Label('emplist'));
    ...
    public function OnFilter($sender) {
        ...
        if ($status == -1)
            $where .= " and  status <> " . Task::TASK_STATUS_CLOSED; // все  незакрытые
        else
    ...
    public function tasklistOnRow($row) {
        ...
        $row->add(new ClickLink('name', $this, 'editOnClick'))->setValue($task->taskname);
        ;
        ...
        $row->add(new Label('cost', number_format($task->cost, 2, '.', '')));
    ...
    public function OnEdit($sender) {
        ...
        $this->_task->save();
        $this->contenttab->editform->editstatus->setOptionList(Task::getStatusList($this->_task->status));
    ...
    public function OpenTask() {
        ...
        $this->updateHistory();
        $emp = array();
        foreach ($this->_task->getAssignedList() as $_emp) {
            $emp[] = $_emp->getShortName();
        }
        $this->contenttab->emplist->setText(implode(", ", $emp));
    }
    ...
    public function updateGantt() {
        ...
        foreach ($items as $item) {
            $col = "#00ff00";
            if ($item->priority == Task::TASK_PRIORITY_HIGH)
                $col = "#ff0000";
            if ($item->priority == Task::TASK_PRIORITY_NORMAL)
                $col = "#00ff00";
            if ($item->priority == Task::TASK_PRIORITY_LOW)
                $col = "#ffdd00";
            if ($item->status == Task::TASK_STATUS_CLOSED)
                $col = "#a0a0a0";
