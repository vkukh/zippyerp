...
class Helper
{
     ...
     * @param mixed $incparent  вкючая отнаследованные  с  родительских  групп
     * @param mixed $onlyhasproductvalues  только  имеющие  установленные   в   продуктах значения
     */
    public static function getProductAttributeListByGroup($group_id, $incparent = false) {
        $list = array();
        if ($group_id == 0)
            return $list;
        $conn = DB::getConnect();
