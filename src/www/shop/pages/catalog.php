...
class Catalog extends Base
{
    ...
    public function __construct($id) {
        ...
        foreach (\ZippyERP\Shop\Entity\Manufacturer::find("manufacturer_id in(select manufacturer_id from shop_products where deleted <> 1 and group_id={$this->group_id})", "manufacturername") as $key => $value) {
        ...
        $this->sfilter->add(new TextInput('pricefrom'))->setText(floor($pr["minp"] / 100));
        $this->sfilter->add(new TextInput('priceto'))->setText(ceil($pr["maxp"] / 100));
        ;
        ...
        $this->sortform->add(new DropDownChoice('sortorder'))->onChange($this, function($sender) {
            $sender->getPageOwner()->catlist->Reload();
        });
        ...
        $this->cartlistpanel->add(new DataView('cartlist', \ZippyERP\Shop\Basket::getBasket(), $this, "onCartRow"));
        $this->add(new Panel('complistpanel'));
        $this->complistpanel->add(new DataView('complist', \ZippyERP\Shop\CompareList::getCompareList(), $this, "onCompRow"));
        ...
        $this->recentlyp->add(new DataView('rlist', new EntityDataSource("\\ZippyERP\\Shop\\Entity\\Product", "  product_id in (" . implode(",", $ra) . ")"), $this, 'rOnRow'));
        if (count($ra) > 0) {
    ...
    public function plistOnRow($row) {
        ...
        $row->add(new BookmarkableLink("simage", "/sp/" . $item->product_id))->setValue("/simage/" . $item->image_id . "/t");
        $row->add(new BookmarkableLink("scatname", "/sp/" . $item->product_id))->setValue($item->productname);
        $row->add(new Label("stopsaled"))->setVisible($item->topsaled == 1);
        $row->add(new Label("snovelty"))->setVisible($item->novelty == 1);
        $row->add(new Label("soldprice", Helper::fm($item->oldprice)))->setVisible($item->oldprice > 0);
        $row->add(new Label("sshortdesc", $item->description));
        $row->add(new Label("sprice", Helper::fm($item->price)));
        $row->add(new TextInput('srated'))->setText($item->rated);
    ...
    public function onCartRow($row) {
        ...
        $row->add(new ClickLink("cartdel", $this, function($sender) {
            $item = $sender->getOwner()->getDataItem();
            \ZippyERP\Shop\Basket::getBasket()->deleteProduct($item->product_id);
        }));
    }
    ...
    public function onCompRow($row) {
        ...
        $row->add(new ClickLink("compdel", $this, function($sender) {
            $item = $sender->getOwner()->getDataItem();
            \ZippyERP\Shop\CompareList::getCompareList()->deleteProduct($item->product_id);
        }));
    }

    public function rOnRow($row) {
        $item = $row->getDataItem();
        $row->add(new BookmarkableLink("rimage", "/sp/" . $item->product_id))->setValue("/simage/" . $item->image_id . "/t");
...
//строка  атрибута
    ...
    protected function beforeRender() {
        ...
        $this->complistpanel->complist->setDataSource(\ZippyERP\Shop\CompareList::getCompareList());
        $this->complistpanel->complist->Reload();
        $this->complistpanel->setVisible(\ZippyERP\Shop\CompareList::getCompareList()->isEmpty() == false);
    }
...
class CatDataSource implements \Zippy\Interfaces\DataSource
{
    ...
    private function getWhere() {
        ...
        $where .= " and price >= " . $form->pricefrom->getText() * 100;
        $where .= " and price <= " . $form->priceto->getText() * 100;
