...
use App\Entity\Item;
use App\Entity\Doc\BankStatement as BS;
 ...
 * Банковская   выписка
 */
class BankStatement extends \App\Pages\Base
{
    ...
    public function __construct($docid = 0) {
        ...
        $this->editdetail->add(new DropDownChoice('editstore', \App\Entity\Store::getList()))->setVisible(false);
        $this->editdetail->add(new CheckBox('editprepayment'))->setChecked(1);

        $this->editdetail->add(new DropDownChoice('editpayment'))->setOptionList(H::getTaxesList());
    ...
    public function saverowOnClick($sender) {
        ...
        if ($doc == 0 && $this->editdetail->editdoc->isVisible()) {
            $this->setError("Не выбран  документ   ");
            return;
        }
        ...
        $entry = new Item();   //используем   класс  проводки  для   строки
        ...
        $entry->store = $this->editdetail->editstore->getValue();
        if ($entry->store > 0) {
            $entry->customername = $this->editdetail->editstore->getValueName();
        }
        $entry->prepayment = $this->editdetail->editprepayment->isChecked();
        $entry->prepayment = $this->editdetail->editprepayment->isChecked();

        $entry->amount = $this->editdetail->editamount->getText();
        $entry->nds = $this->editdetail->editnds->getText();
    ...
    private function checkForm() {
        ...
        if (count($this->_list) == 0) {
            $this->setError("Не  введены строки");
        }
    ...
    public function typeOnClick($sender) {
        $this->editdetail->editstore->setVisible(false);
        ...
        if ($type == BS::OUT_CARD) {
            ...
            $this->editdetail->editstore->setVisible(true);
        }
    ...
    public function OnDocAutocomplete($sender) {
        ...
        $answer = array();
        $conn = \ZDB\DB::getConnect();
        $sql = "select document_id,document_number from documents where document_number  like '%{$text}%' and document_id <> {$this->_doc->document_id} and state = " . Document::STATE_EXECUTED . "  order  by document_id desc  limit 0,20";
        $rs = $conn->Execute($sql);
        foreach ($rs as $row) {
    ...
    public function OnDocChange($sender) {
        ...
        if ($doc instanceof Document) {
            $this->editdetail->editamount->setText(H::famt($doc->amount));
            $this->editdetail->editnds->setText(H::famt($doc->headerdata['nds']));
        }
