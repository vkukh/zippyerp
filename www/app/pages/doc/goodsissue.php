...
class GoodsIssue extends \App\Pages\Base
{
                ...
                if ($basedoc instanceof Document) {
                        ...
                        foreach ($order->detaildata as $item) {
                            ...
                            if (count($stlist) == 0) {
                                ...
                                foreach ($stlist as $st) {
                                    $st->price = $item['price'];
                                    if (H::usends()) {
                                        $st->pricends = $st->price;
                                        $st->price = H::famt($st->pricends - $st->pricends * H::nds(true));
                                    }
                    ...
                    if ($basedoc->meta_name == 'Invoice') {
                        ...
                        foreach ($basedoc->detaildata as $item) {
                            $stlist = Stock::pickup($basedoc->headerdata['store'], $item['item_id'], $item['quantity']);
                            if (count($stlist) == 0) {
                                $notfound[] = $item['itemname'] . "({$item['quantity']}шт) ";
                            } else {
                                foreach ($stlist as $st) {
    ...
    public function OnAutoItem($sender) {
        ...
        return Stock::findArrayEx("store_id={$store_id} and qty>0 and acc_code in('281','26')     and (itemname like {$text} or item_code like {$text}) ");
    }
