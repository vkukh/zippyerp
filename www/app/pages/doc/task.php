...
class Task extends \App\Pages\Base
{
    ...
    public function __construct($docid = 0) {
        ...
        $this->editdetail2->add(new TextInput('editpricends2'));
        ...
        $this->editdetail3->add(new DropDownChoice('editemp', Employee::findArray('emp_name', "detail like '%<fired>0</fired>%'  ", 'emp_name')));
        ...
        $this->editdetail4->add(new DropDownChoice('editeq', CAsset::findArray("ca_name", "acc_code='106' or acc_code='104'", "ca_name")));
    ...
    public function detailOnRow($row) {
        ...
        $row->add(new Label('price', H::famt($service->price)));
        $row->add(new Label('pricends', H::famt($service->pricends)));

        $row->add(new Label('amount', H::famt($service->amount)));
    ...
    public function editOnClick($sender) {
        ...
        $this->editdetail->editpricends->setText($service->pricends);
    ...
    public function saverowOnClick($sender) {
        ...
        $service->pricends = $this->editdetail->editpricends->getText();
    ...
    public function detail2OnRow($row) {
        ...
        $row->add(new Label('price2', H::famt($item->price)));
        $row->add(new Label('pricends2', H::famt($item->pricends)));

        $row->add(new Label('amount2', H::famt($item->amount)));
    ...
    public function additemOnClick($sender) {
        ...
        $this->editdetail2->editpricends2->setText(0);
    }
    ...
    public function edit2OnClick($sender) {
        ...
        $this->editdetail2->editpricends2->setText($item->pricends);
    ...
    public function saverow2OnClick($sender) {
        ...
        $stock->pricends = $this->editdetail2->editpricends2->getText();
    ...
    public function detail4OnRow($row) {
        ...
        $row->add(new Label('eq_name', $eq->ca_name));
    ...
    public function delete4OnClick($sender) {
        ...
        $this->_emplist = array_diff_key($this->_eqlist, array($eq->ca_id => $this->_eqlist[$eq->ca_id]));
        $this->docform->detail4->Reload();
    }
    ...
    public function OnAutoItem($sender) {
        ...
        return Stock::findArrayEx("store_id={$store_id} and acc_code='281'  and (itemname like {$text} or item_code like {$text}) ");
    }

    public function OnAutoItem5($sender) {
        $store_id = $this->docform->store->getValue();
        $text = Item::qstr('%' . $this->editdetail5->edititem5->getText() . '%');
        return Stock::findArrayEx("store_id={$store_id} and acc_code in('201','204') and (itemname like {$text} or item_code like {$text}) ");
    }

    public function OnChangeItem($sender) {
        $id = $sender->getKey();
        $stock = Stock::load($id);
        $this->editdetail2->qty->setText(H::fqty($stock->qty));
        ...
        if (H::usends()) {
            $this->editdetail2->editpricends2->setText(H::famt($price));
            $this->editdetail2->editprice2->setText(H::famt($price - $price * H::nds(true)));
        }




        $this->updateAjax(array('qty', 'editprice2', 'editpricends2'));
    }
