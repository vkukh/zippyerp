...
class StockList extends \App\Pages\Base
{
    ...
    public function __construct() {
        ...
        $itt = array('281' => 'Товары на складе', '282' => 'Товары в торговле', '201' => 'Сырье и материалы', '22' => 'МБП', '203' => 'Топливо', '204' => 'Тара', '207' => 'Запчасти',);
        $this->filter->add(new DropDownChoice('itemtype', $itt, 0));
    ...
    public function itemlistOnRow($row) {
        ...
        $row->add(new Label('partion', H::famt($stock->partion)));
        ...
        $row->add(new Label('acc_name', $stock->acc_name));
        $plist = array();
        if ($item->price1 > 0)
...
class ItemDataSource implements \Zippy\Interfaces\DataSource
{
    ...
    private function getWhere() {
        ...
        $type = $form->itemtype->getValue();
        if ($store > 0) {
            $where = $where . " and store_id=" . $store;
        }
        if ($type > 0) {
            $where = $where . " and acc_code='{$type}'";
        }
