...
class TaskList extends \App\Pages\Base
{
    ...
    public function tasklistOnRow($row) {
        ...
        $row->taskstatus->setText(Document::getStateName($task->state));
        ...
        $row->add(new ClickLink('taskcancel'))->onClick($this, 'taskcancelOnClick');

        if ($task->state == Document::STATE_CANCELED || $doc->state == Document::STATE_EDITED || $doc->state == Document::STATE_NEW) {
            $row->taskedit->setVisible(true);

            $row->taskcancel->setVisible(false);
        } else {
            $row->taskedit->setVisible(false);

            $row->taskcancel->setVisible(true);
        }
    }
